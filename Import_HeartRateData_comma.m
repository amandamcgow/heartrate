...
elapsed_min = [0,31.1]; % [exercise start, exercise end] put time elapsed from run sheet in minutes [5,25] - indicates exercise was from min 5 to min 25 of entire exp session
        ...
        participant = num2str(sprintf('%03d', participantnumber));
            for condition = {'C'}
                ...
                if (exist(filein, 'file') > 0) % if file exists
                   ...
                   % Export Data to File
                   filout = strcat(def.path, 'HRdata\', cell2mat(prefix), participant, cell2mat(condition), '_clean.csv');

                   fid = fopen(filout, 'wt');
                   %fprintf(fid, '%s\n,', strcat(cell2mat(prefix), participant, cell2mat(condition)));
                   fprintf(fid, '%s%s\n', 'Time_sec', 'Heart_rate'); % header                   
                   dlmwrite(filout, new_mat, '-append')
